...
final class UpdateCoordinator: AppUpdateCoordinating {
    ...
    static let homebrewUpdateCommand = "brew upgrade nehir"
    ...
    init(
        ...
        releaseService: any GitHubReleaseFetching = GitHubReleaseService(
            userAgent: "Nehir/\(Bundle.main.appVersion ?? BuildVersion.display)"
        ),
        currentVersionProvider: @escaping () -> ReleaseVersion? = { Bundle.main.releaseVersion },
        currentVersionStringProvider: @escaping () -> String = { Bundle.main.appVersion ?? BuildVersion.display },
        ...
        guard let currentVersion = currentVersionProvider() else {
            if source == .manual {
                infoAlertPresenter(
                    ...
                    "Nehir could not determine its current version."
                )
            }
            return
        }
    ...
    private func handleFetchedRelease(
        ...
        guard let latestVersion = release.version else {
            if source == .manual {
                infoAlertPresenter(
                    "Could Not Check for Updates",
                    "GitHub returned an update tag Nehir could not parse."
                )
            }
            return
        }
        ...
        if latestVersion <= currentVersion {
            ...
            if source == .manual {
                infoAlertPresenter(
                    "You're Up to Date",
                    "Nehir \(currentVersionStringProvider()) is already the latest available release."
                )
            }
            return
        }
